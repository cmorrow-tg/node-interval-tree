  ...
  */
  ...
  private _updateMaxAfterRightRotate() {
    ...
    if (left.left === undefined && left.right !== undefined) {
      ...
      left.max = Math.max(
        Math.max((left.left as Node<T>).max, (left.right as Node<T>).max),
        thisParentLeftHigh
      );
    }
    ...
    if (this.left === undefined && this.right !== undefined) {
      ...
      this.max = Math.max(
        Math.max((this.left as Node<T>).max, (this.right as Node<T>).max),
        thisHigh
      );
    }
    ...
    parent.max = Math.max(
      Math.max((parent.left as Node<T>).max, (parent.right as Node<T>).max),
      parent.getNodeHigh()
    );
  }
  ...
  */
  ...
  private _updateMaxAfterLeftRotate() {
    ...
    if (right.left === undefined && right.right !== undefined) {
      ...
      right.max = Math.max(
        Math.max((right.left as Node<T>).max, (right.right as Node<T>).max),
        thisParentRightHigh
      );
    }
    ...
    // Update max of parent (y in first case, x in second)
    parent.max = Math.max(
      Math.max((parent.left as Node<T>).max, right.max),
      parent.getNodeHigh()
    );
  }
      ...
      if (node === undefined) {
        ...
        if (isSame(node.records[0], record)) {
          ...
          if (this.root.key === node.key) {
            ...
            // root's parent role
            const rootParent = new Node<T>(this, {
              low: record.low,
              high: record.low,
            } as T);
...
export default class DataIntervalTree<T> {
  ...
  public searchOnly(low: number, high: number, includedData: Array<T>) {
    return this.tree
      .search(low, high)
      .map(v => v.data)
      .filter(data => {
        return includedData.includes(data);
      });
  }
